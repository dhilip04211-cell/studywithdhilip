...
async function loadTesseract() {
  ...
  if (tesseractLoading) {
    await new Promise(resolve => {
      const iv = setInterval(() => { if (tesseractReady) { clearInterval(iv); resolve(); } }, 200);
    });
...
function MissingFieldBanner({ editData, onDismiss }) {
  ...
  REQUIRED_COLS.forEach(ci => {
    const v = (editData[ci] || "").toString().trim();
    if (!v) missing.push(HEADERS[ci]);
  });
...
export default function CRLMPEditor() {
  ...
  const search = async () => {
    ...
    setShowWarn(false); setWarnDismissed(false);
    try {
  ...
  const applyCrop = () => {
    ...
    if (cropRect && cropRect.w > 10 && cropRect.h > 10) {
      c.width = cropRect.w * scaleX; c.height = cropRect.h * scaleY;
      const imgEl = new Image();
      imgEl.onload = () => {
        c.getContext("2d").drawImage(imgEl, cropRect.x*scaleX, cropRect.y*scaleY, c.width, c.height, 0, 0, c.width, c.height);
  ...
  const runOCR = async (blob) => {
    ...
    try {
      ...
      if (txt) {
        ...
        setOcrError("No text detected. Try cropping closer to the text."); setOcrMode("crop");
      }
    } catch (e) {
      setOcrError("OCR error: " + e.message); setOcrMode("crop");
    } finally {
      setOcrProcessing(false); setOcrProgress(0); setOcrStatus("");
    }
  };
    ...
    <div style={S.root}>
      ...
      <div style={S.body}>
          ...
          <div style={S.searchWrap}>
            <div className="searchRow" style={S.searchRow}>
              ...
              <div style={S.fg}>
                ...
                <input ref={caseYearRef} style={{ ...S.inp, width:92 }}
        ...
        {result && showWarn && !warnDismissed && (
          <MissingFieldBanner
            editData={editData}
            onDismiss={() => { setWarnDismissed(true); setShowWarn(false); }}
          />
        )}
          ...
          <section style={{ ...S.card, padding:0, border:`1.5px solid #C9A84C` }}>
            <div className="resHead" style={S.resHead}>
              <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
                ...
                {missingCount > 0 && (
                  <span style={S.missBadge}>
                    <span className="miss-dot" />
                    {missingCount} field{missingCount > 1 ? "s" : ""} empty
                  </span>
                )}
              </div>
              ...
              {HEADERS.map((hdr, ci) => {
                ...
                return (
                  <div key={ci} className="field-card"
                    style={{ ...S.fCard,
                             ...(isMod  ? S.fCardMod   : {}),
                             ...(isEmpty ? S.fCardEmpty : {}),
                             animationDelay:`${ci*25}ms` }}>
                    <div style={S.fTop}>
                      <span style={{ ...S.fLbl, ...(isEmpty ? { color:"#e74c3c" } : {}) }}>
                        {isEmpty && <span className="miss-dot" />}
                        {hdr}
                      </span>
            ...
            <div className="saveBar" style={S.saveBar}>
              ...
              {missingCount > 0 && !warnDismissed && (
                <span style={S.warnSaveHint}>⚠ {missingCount} empty field{missingCount>1?"s":""}</span>
              )}
              ...
              <div style={{ position:"relative", userSelect:"none" }}>
                ...
                <div style={{ position:"relative", lineHeight:0, touchAction:"none" }}
                  ...
                  <img ref={cropImgRef} src={capturedUrl} alt="captured"
                    style={{ width:"100%", maxHeight:300, objectFit:"contain", display:"block", cursor:"crosshair" }}
                    draggable={false} />
                  {cropRect && cropRect.w > 4 && cropRect.h > 4 && (
                    <div style={{ position:"absolute", left:cropRect.x, top:cropRect.y, width:cropRect.w, height:cropRect.h,
                                  border:"2px solid #C9A84C", background:"rgba(201,168,76,0.15)", pointerEvents:"none" }} />
                  )}
                </div>
