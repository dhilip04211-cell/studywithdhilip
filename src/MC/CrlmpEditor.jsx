...
function injectCSS() {
  ...
  s.textContent = `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .gis-btn { cursor:pointer; font-family:inherit; transition:all .15s; }
    .gis-btn:hover { opacity:.85 !important; }
  `;
...
export default function CRLMPEditor() {
  ...
  /* ── Load Google Identity Services (fixes 403 disallowed_useragent) ─────── */
  useEffect(() => {
    // Load GIS script
    ...
    gis.onload = () => {
      tokenClient.current = window.google.accounts.oauth2.initTokenClient({
        ...
        callback: (resp) => {
          ...
          // Token expires in ~1hr — auto-refresh
          setTimeout(() => {
            tokenRef.current = "";
            setToken("");
    ...
    // Load Sheets API (read-only, uses API key)
    fetchSheetNames();
  }, []);
  ...
  const fetchSheetNames = useCallback(async () => {
    try {
      ...
      setError("Cannot load sheet tabs. Check Sheet ID & API Key.");
    }
  }, []);
  ...
  /* ── Sign-in via GIS token popup ─────────────────────────────────────────── */
  const signIn = () => {
    if (!gisReady) { setError("Google Sign-In not ready yet. Wait a moment."); return; }
  ...
  const save = async () => {
    if (!result) return;
    if (!tokenRef.current) { setError("Sign in with Google first to save changes."); return; }
    setSaving(true); setSaveMsg(""); setError("");
    try {
      const data = [];
      // Address always saved
      ...
      setModCols(new Set());
      // Auto-focus CRLMP number field after save
  ...
  /* persist year */
  useEffect(() => { if (caseYear) localStorage.setItem("crlmp_year", caseYear); }, [caseYear]);
  ...
  const runOCR = async (blob) => {
    ...
    try {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              { type: "text",  text: "Extract ALL text visible in this image. Return ONLY the raw text exactly as it appears — no explanation, no formatting. This is likely an address or legal document field." }
            ]
          }]
        })
      });
      ...
      if (txt.trim()) {
        ...
        setOcrError("No text detected. Try again with a clearer photo.");
      }
    } catch (e) {
    ...
    <div style={S.root}>
        ...
        <div style={S.hInner}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            {token
              ...
              : <button className="gis-btn" style={S.signBtn} onClick={signIn} disabled={!gisReady}>
      ...
      <div style={S.body}>
        ...
        <section style={S.card}>
          <p style={S.secLabel}>SELECT SHEET TAB</p>
          <div style={S.tabBar}>
              ...
              {HEADERS.map((hdr, ci) => {
                ...
                return (
                  <div key={ci} className="field-card"
                    style={{ ...S.fCard, ...(isMod ? S.fCardMod : {}), animationDelay:`${ci*28}ms` }}>
                    <div style={S.fTop}>
...
const S = {
  ...
  signBtn:  { padding:"8px 18px", background:"transparent", border:`1.5px solid ${G}`, color:G, borderRadius:6, fontSize:12, fontFamily:"inherit" },
  ...
  searchBtn: { padding:"10px 28px", background:`linear-gradient(135deg,${G},#8B5E0A)`, border:"none", borderRadius:8, color:"#fff", fontWeight:700, fontSize:14, cursor:"pointer", display:"flex", alignItems:"center", gap:8, fontFamily:"inherit", whiteSpace:"nowrap", transition:"opacity .2s" },
  ...
  ocrErr:     { color:"#c0392b", fontSize:12, padding:"8px 16px", background:"#fff0f0", margin:"0" },
