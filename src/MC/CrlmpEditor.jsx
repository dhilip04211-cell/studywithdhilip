...
const LS_TA_CACHE  = "crlmp_ta_cache"; // tanglish→tamil cache
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   Returns up to 6 Tamil suggestions for a given tanglish word.
   Results are cached in localStorage to avoid repeat calls.
══════════════════════════════════════════════════════════════════════════ */
const pendingRequests = {};   // word → Promise  (dedup in-flight)
...
async function fetchTamilSuggestions(word) {
  ...
  /* dedup: if same word already in-flight, wait for it */
...
- Output ONLY the raw JSON array, no explanation, no markdown, no quotes around the array.
Example input: amudhan
Example output: ["அமுதன்","அமுதன","அமுதன்ஸ்","அமுது","அமுதா","அமுதகன்"]`,
...
function AddressField({ value, onChange, onCamera, onFile, sheetAddresses }) {
  ...
  const handleChange = (e) => {
    ...
    /* if current segment is pure English letters → tanglish mode */
    if (word.length >= 2 && /^[a-zA-Z]+$/.test(word)) {
      ...
      debounceRef.current = setTimeout(async () => {
        if (word.toLowerCase() === lastWordRef.current) return;
      ...
      if (q.length >= 3) {
        ...
        if (hits.length) { setAcList(hits); setAcOpen(true); setAcRect(getDropRect()); return; }
      }
      setAcOpen(false);
    }
  };
  ...
  const selectAC = (addr) => {
    onChange(addr); setAcOpen(false);
    const hist = ls.getArr(LS_ADDR_HIST);
    ls.setArr(LS_ADDR_HIST, [addr,...hist.filter(a=>a!==addr)].slice(0,20));
  };

  const handleKeyDown = (e) => {
    if (sugOpen && (sugList.length || sugLoading)) {
  ...
  useEffect(()=>{
    ...
    const update=()=>{ const r=getDropRect(); if(sugOpen)setSugRect(r); if(acOpen)setAcRect(r); };
  ...
  return (
    <div style={{ position:"relative" }}>
      ...
      <div style={S.tanglishHint}>
        ...
        <span style={S.tanglishTip}>type English → get Tamil (Tab/→ to pick)</span>
      </div>
      ...
      {/* ── TANGLISH SUGGESTION POPUP ── */}
      {sugOpen && sugRect && (
        <div className="sug-popup" style={{
          ...S.sugPop,
          position:"fixed", top:sugRect.top, left:sugRect.left, width:sugRect.width, zIndex:9998,
        }}>
          <div style={S.sugHeader}>
            <span style={S.sugTitle}>
              {sugLoading
                ? <><span style={S.miniSpin}/>  AI transliterating…</>
                : `Tamil suggestions for "${lastWordRef.current}"`
              }
            </span>
            <span style={S.sugNav}>Tab / → insert · ↑↓ cycle · Esc close</span>
          </div>
          <div style={S.sugRow}>
            {sugLoading && !sugList.length && (
              [1,2,3].map(i=>(
                <div key={i} style={S.sugSkeleton}/>
              ))
            )}
            {sugList.map((s,i)=>(
              <button key={i}
          ...
          {!sugLoading && sugList.length===0 && (
            <div style={S.sugEmpty}>No suggestions — keep typing or type in Tamil directly</div>
          )}
        </div>
      )}
      ...
      {acOpen && acRect && acList.length>0 && (
        <div className="ac-dropdown" style={{
          ...S.acDrop,
          position:"fixed", top:acRect.top, left:acRect.left, width:acRect.width, zIndex:9997,
        }}>
          <div style={S.acHeader}><span style={S.acHeaderTxt}>📋 Sheet suggestions</span></div>
          {acList.map((addr,i)=>(
...
function DateField({ value, isMod, onChange }) {
  ...
  return (
    <input style={{...S.inpInline,width:"100%",boxSizing:"border-box",...(isMod?{borderColor:"#C9A84C",background:"#fffbf0"}:{})}}
      value={draft} onChange={handleChange} inputMode="numeric" enterKeyHint="done"
...
export default function CRLMPEditor() {
  ...
  const cropImgRef=useRef(null), videoRef=useRef(null), canvasRef=useRef(null);
  const streamRef=useRef(null), fileRef=useRef(null);
  const caseNumRef=useRef(null), caseYearRef=useRef(null);
  ...
  const search=async()=>{
    ...
    try{
      ...
      for(let i=1;i<rows.length;i++){
        const cn=normStr(rows[i][COL.CASE]||""), qn=normStr(q);
  ...
  const save=async()=>{
    ...
    try{
      const data=[];
      modCols.forEach(ci=>data.push({range:`${activeSheet}!${toA1(result.rowIndex,ci)}`,values:[[editData[ci]]]}));
  ...
  const capturePhoto=()=>{
    ...
    stopStream();
    c.toBlob(blob=>{const url=URL.createObjectURL(blob);setCapturedBlob(blob);setCapturedUrl(url);setCropRect(null);setCropStart(null);setOcrMode("crop");},"image/jpeg",.95);
  };
  ...
  const applyCrop  =()=>{
    if(!capturedBlob)return; const img=cropImgRef.current;if(!img)return; const c=canvasRef.current;if(!c)return;
    const sx=img.naturalWidth/img.getBoundingClientRect().width, sy=img.naturalHeight/img.getBoundingClientRect().height;
    if(cropRect&&cropRect.w>10&&cropRect.h>10){
      c.width=cropRect.w*sx;c.height=cropRect.h*sy;
      const imgEl=new Image();
      imgEl.onload=()=>{ c.getContext("2d").drawImage(imgEl,cropRect.x*sx,cropRect.y*sy,c.width,c.height,0,0,c.width,c.height); c.toBlob(b=>runOCR(b),"image/jpeg",.95); };
      imgEl.src=capturedUrl;
    }else{runOCR(capturedBlob);}
  };
  const stopStream=()=>{streamRef.current?.getTracks().forEach(t=>t.stop());streamRef.current=null;};
  const pickFile=(e)=>{
    const f=e.target.files[0];if(!f)return;e.target.value="";
    const url=URL.createObjectURL(f);setCapturedBlob(f);setCapturedUrl(url);
    setCropRect(null);setCropStart(null);setOcrOpen(true);setOcrError("");setOcrProcessing(false);setOcrMode("crop");
  };
  const runOCR=async(blob)=>{
    setOcrMode("processing");setOcrProcessing(true);setOcrError("");setOcrProgress(0);setOcrStatus("Loading Tesseract engine…");
    try{
      const worker=await loadTesseract(); setOcrStatus("Recognising text…");
      const imgUrl=URL.createObjectURL(blob);
      const{data}=await worker.recognize(imgUrl,{},{text:true}); setOcrProgress(100);
      URL.revokeObjectURL(imgUrl);
      const txt=data.text?.trim()||"";
      if(txt){ setEditData(p=>({...p,[COL.ADDR]:txt})); setModCols(p=>new Set([...p,COL.ADDR])); setOcrOpen(false); if(capturedUrl)URL.revokeObjectURL(capturedUrl); }
      else{setOcrError("No text detected. Try cropping closer.");setOcrMode("crop");}
    }catch(e){setOcrError("OCR error: "+e.message);setOcrMode("crop");}
    finally{setOcrProcessing(false);setOcrProgress(0);setOcrStatus("");}
  };
  const closeOcr=()=>{
        ...
        <div className="hInner" style={S.hInner}>
          ...
          <div className="authArea" style={S.authArea}>
            {isLoggedIn?(
              ...
              <button style={S.signBtn} onClick={signIn} disabled={!gisReady}>{gisReady?"🔐 Sign In with Google":"⏳ Loading…"}</button>
            )}
          </div>
        </div>
      </header>
        ...
        {result&&(
          <section style={{...S.card,padding:0,border:`1.5px solid #C9A84C`}}>
            ...
            <div className="grid" style={S.grid}>
              {HEADERS.map((hdr,ci)=>{
                const isMod =modCols.has(ci), isDate=DATE_COLS.has(ci);
                const isAddr=ci===COL.ADDR,   isRO=READONLY.has(ci);
                ...
                return(
                  <div key={ci} className="field-card"
                    ...
                    {isAddr?(
                      <AddressField value={editData[ci]||""} onChange={v=>markMod(ci,v)}
                        onCamera={openCamera} onFile={()=>fileRef.current?.click()} sheetAddresses={sheetAddresses}/>
                    ):isDate?(
          ...
          <div style={S.ocrCard}>
            <div style={S.ocrHead}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={S.ocrTitle}>{ocrMode==="crop"?"✂️ Crop & Scan":ocrMode==="processing"?"🔍 Scanning…":"📷 Scan Address"}</span>
            ...
            {ocrMode==="processing"?(
              <div style={S.ocrCenter}>
                ...
                <div style={{width:"80%",marginTop:8}}>
                  <div className="ocr-progress-bar">
                    <div className="ocr-progress-fill" style={{width:ocrProgress>0?`${ocrProgress}%`:"60%",animation:ocrProgress===0?"pulse 1.5s ease-in-out infinite":"none"}}/>
                  </div>
                  <p style={{fontSize:10,color:"#bbb",textAlign:"center",marginTop:4,letterSpacing:"0.08em"}}>RUNNING IN BROWSER · NO DATA SENT TO SERVER</p>
                </div>
              </div>
            ):ocrMode==="crop"&&capturedUrl?(
              <div style={{position:"relative",userSelect:"none"}}>
                <p style={{fontSize:11,color:"#888",textAlign:"center",padding:"8px 0 4px",background:"#f8f6f1",letterSpacing:"0.05em"}}>DRAG to select · leave empty for full image</p>
                <div style={{position:"relative",lineHeight:0,touchAction:"none"}}
                  ...
                  {cropRect&&cropRect.w>4&&cropRect.h>4&&(
                    <div style={{position:"absolute",left:cropRect.x,top:cropRect.y,width:cropRect.w,height:cropRect.h,border:"2px solid #C9A84C",background:"rgba(201,168,76,.15)",pointerEvents:"none"}}/>
                  )}
                </div>
                {ocrError&&<p style={{color:"#c0392b",fontSize:12,padding:"6px 14px",background:"#fff0f0",textAlign:"center"}}>{ocrError}</p>}
                <div style={{display:"flex",gap:8,padding:"10px 12px"}}>
                  ...
                  <button style={{...S.capBtn,flex:2}} onClick={applyCrop}>{cropRect&&cropRect.w>10?"✂️ Crop & Scan":"🔍 Scan Full"}</button>
                </div>
              ...
              <div style={{position:"relative"}}>
                <div style={{position:"relative",lineHeight:0}} onClick={tapFocus}>
                  ...
                  {focusRing.show&&<div style={{position:"absolute",left:focusRing.x-22,top:focusRing.y-22,width:44,height:44,border:`2px solid ${G}`,borderRadius:"50%",pointerEvents:"none"}}/>}
                  <p style={{position:"absolute",bottom:6,left:0,right:0,textAlign:"center",fontSize:10,color:"#fff9",letterSpacing:"0.1em"}}>TAP TO FOCUS</p>
                </div>
                <canvas ref={canvasRef} style={{display:"none"}}/>
                {ocrError&&<p style={{color:"#c0392b",fontSize:12,padding:"8px 14px",background:"#fff0f0",textAlign:"center"}}>{ocrError}</p>}
                <div style={S.ocrBtns}>
                  <button style={S.galleryBtn} onClick={()=>fileRef.current?.click()}>🖼 Gallery</button>
...
const S={
  ...
  /* Sheet autocomplete */
  acDrop:    {background:"#fff",border:`1.5px solid ${G}`,borderRadius:"0 0 10px 10px",boxShadow:"0 8px 24px rgba(0,0,0,.12)",maxHeight:220,overflowY:"auto"},
  ...
  acItem:    {display:"flex",alignItems:"flex-start",gap:10,padding:"9px 12px",background:"none",border:"none",width:"100%",textAlign:"left",cursor:"pointer",fontFamily:"inherit",borderBottom:"1px solid #f8f4ee",transition:"background .12s"},
  ...
  camBtn:    {flex:1,padding:"8px 14px",background:"#f0f6ff",border:"1.5px solid #c0d8ff",borderRadius:6,color:"#3B6BF5",fontSize:12,cursor:"pointer",fontFamily:"inherit",transition:"all .15s",textAlign:"center"},
